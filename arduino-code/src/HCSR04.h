...
public:
    /**
     * @param triggerPin  Digital pin that is used for controlling sensor
     * (output).
     * @param maxDistanceCm  Maximum distance sensor can measure, defaults to 4m
     * for HC-SR04. You might want to set this value if you are using different
     * sensor than HC-SR04 or if you don't care about distances larger than
     * whatever you will set it to (therefore reducing time it takes for a
     * single measurement).
     ...
     *   By default, there is no limit on time (only on distance). By defining
     * timeout, you are again limiting the distance.
     */
    UltraSonicDistanceSensor(byte triggerPin,
                             unsigned short maxDistanceCm = 400,
                             unsigned long maxTimeoutMicroSec = 0);

    /**
     * Measures distance by sending ultrasonic waves and measuring time it takes
     * them to return.
     * @returns Distance in centimeters, or negative value if distance is
     * greater than 400cm.
     */
    ...
    /**
     * Measures distance by sending ultrasonic waves and measuring time it takes
     * them to return. Measurement can not exceed duration calculated with
     * maxDistanceCm or maxTimeoutMicroSec.
